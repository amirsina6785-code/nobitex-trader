...
class TradingViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun connect(url: String, key: String) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                repo.connect(key)
                    .onSuccess {
                        store.save(url.trim(), key)
                        connection.value = UiState.Success(it)
                        refreshAll()
                    }
                    .onFailure {
                        ApiClient.clear()
    ...
    fun refreshStatus() {
        viewModelScope.launch {
            ...
            repo.getBotStatus()
                .onSuccess {
                    status.value = UiState.Success(it)
                }
                .onFailure {
                    status.value =
                        UiState.Error(
    ...
    fun syncWallet() {
        viewModelScope.launch {
            ...
            repo.syncWallet()
                .onSuccess {
                    wallet.value = UiState.Success(it)
                    message.value = "کیف پول همگام شد."
                }
                .onFailure {
                    wallet.value =
                        UiState.Error(
